...
global.numenLoad = function (script, breakpoints, asName) {
...
function runDebuggerLoop (exec_state) {
    ...
    while (!exit) {
        try {
            var res = null;
            // debugger commands
            var req = JSON.parse(fs.readFileSync("/tmp/numen_pipe", "utf8"));
...
global.numenWriteInPlace = function (text, preserve) {
    clientSend({ 'in-place' : text, 'preserve-p' : preserve ? true : null });
}

global.numenReportValue = function (val, depth, batch) {
    clientSend({ 'evaluation' : clientValue(val, depth, batch) });
}

global.numenSlimeEval = function (callback, string, package) {
    var id = clientCallID++;
    clientCallbacks[id] = callback;
    clientSend({ 'slime-eval' : string, 'callback-id' : id, 'package' : package });
}

global.numenEmacsEval = function (callback, string) {
    var id = clientCallID++;
    clientCallbacks[id] = callback;
