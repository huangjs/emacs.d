...
(eval-when-compile
  (defmacro wlet (binding &rest body)
    "Like `let', but accepts only one binding and adds a null check."
  ...
  (defmacro hget (table key)
    "Like `gethash' but does nothing if TABLE is nil and expects
KEY to be a symbol that it converts to a string. If KEY is a
keyword then the leading colon is dropped from the string.

This macro is for making hashtable access easier for hard-coded keys."
...
(defvar numen-debugger-pipe "/tmp/numen_pipe" "Named pipe through which Numen sends blocking messages to the debugger loop in Node.")
...
(defvar numen-server-message-handlers nil "List of functions that get called with a single argument MSG when the Numen client receives an unrecognized message from the server. The first one that returns non-nil is considered to have handled the message.")
(defvar numen-startup-hook nil "Hook that gets called after the Numen REPL and server have been started.")
...
;;; for Numen buffers that we'd like to keep in a preferred place
(defvar numen-sticky-window nil)
...
(define-derived-mode numen-mode nil "Numen"
  "Major mode for interacting with V8 via Node.js from a REPL.
...
\\{numen-mode-map}"
  ...
  (set (make-local-variable 'numen-sticky-window) (selected-window))
...
(defun run-numen (&optional compiler-function)
  (interactive)
  (let ((invocation-directory default-directory))
    (with-current-buffer (get-buffer-create "*Numen*")
      ...
      (unless (file-exists-p numen-debugger-pipe)
        (shell-command (format "mkfifo %s" numen-debugger-pipe)))
      (when numen-server-process
        (message "Restarting...")
        (numen-terminate))
      (numen-enter-mode)
      (set (make-local-variable 'numen-input-compiler-function) (or compiler-function 'identity))
      (numen-insert-prompt)
       ...
       (cond ((null ,repl) (message "Numen is not running"))
             (t (with-current-buffer ,repl
                  (when (get-buffer-window)
                    (setq numen-sticky-window (get-buffer-window)))
                  ,@body))))))
...
(defun numen-launch-server ()
  (let* ((server-buffer (generate-new-buffer " *numen-server*"))
         (boot (concat numen-directory "numen.js"))
         (args (list "--expose_debug_as=v8debug" "--prof" "--prof_lazy" "--log" boot))
         (proc (let ((process-connection-type nil)) ; use a pipe, not a PTY. see elisp reference.
                 (apply 'start-process "node" server-buffer "node" args))))
...
(defun numen-send-debugger-msg (str)
  (interactive)
  (assert (file-exists-p numen-debugger-pipe))
  (with-temp-buffer
    (insert str)
    (condition-case nil
        (write-region (point-min) (point-max) numen-debugger-pipe nil :no-message)
      (quit (numen-output "Unable to communicate with debugger.\n")))))
...
(defun numen-send-request (req &optional stuff buffer-id)
  ...
  (if (numen-process-running-p numen-server-process)
      (let ((json (json-encode (append req (list :stuff (append stuff (list :buffer buffer-id)))))))
        (cond ((in-debugger-p) (numen-send-debugger-msg json))
  ...
  (let ((msg nil))
    (while (setq msg (numen-read-server-message))
      (acond ((stringp msg) (numen-output msg 'numen-console-face))
             ...
             (t (unless (loop for handler in numen-server-message-handlers do
...
(defun numen-enter-minor-mode ()
  (unless numen-buffer-id ; do this once
    ...
    (let ((repl-buffer (with-repl-buffer (current-buffer))))
      ...
      (let ((repl-window (with-repl-buffer numen-sticky-window))
            (w (next-window)))
        (set (make-local-variable 'numen-sticky-window)
...
(defun numen-exit-minor-mode ()
  (when numen-buffer-id ; do this once
    ...
    (kill-local-variable 'numen-sticky-window)))
...
(defun numen-fold (id val depth)
  ...
  (lexical-let ((changed-p nil))
    (numen-walk-val
     (lambda (v d ignore)
       (wlet (view (numen-view id))
         (when (and (= d depth) (> (length (hget v :vals)) 0) (not (numen-hiding-p view (vkey v))))
...
(defun* numen-display-in-sticky-window (&optional (buffer (current-buffer)))
  "If there is a buffer-local variable NUMEN-STICKY-WINDOW, and
if BUFFER is not already in a window, display it in
NUMEN-STICKY-WINDOW as long as that window is live and not
dedicated. Otherwise display BUFFER in whatever window
`pop-to-buffer' chooses. After displaying BUFFER, rebind
NUMEN-STICKY-WINDOW to the selected window."
  (with-current-buffer buffer
    (when numen-sticky-window
      (when (and (null (get-buffer-window buffer))
                 (window-live-p numen-sticky-window)
                 (not (window-dedicated-p numen-sticky-window)))
        (set-window-buffer numen-sticky-window buffer))
      (pop-to-buffer buffer)
      (setq numen-sticky-window (selected-window)))))

(defun* numen-cycle-window (&optional (buffer (current-buffer)))
  "If there is a buffer-local variable NUMEN-STICKY-WINDOW, set
it to `next-window' and display BUFFER in that window. Otherwise
simply move current buffer to next window."
  (interactive)
  (with-current-buffer buffer
    (let ((w^ (or numen-sticky-window (get-buffer-window) (selected-window))))
      (loop for w = (next-window w^) until (or (not (window-dedicated-p w)) (eq w w^))
            do (setq w (next-window w))
            finally do
            (cond (numen-sticky-window
                   (setq numen-sticky-window w)
                   (replace-buffer-in-windows buffer)
                   (numen-display-in-sticky-window buffer))
